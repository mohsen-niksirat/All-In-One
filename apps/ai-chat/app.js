...
I18N.register('fa', {
    ...
    chat_merged: '{n} پیام جدید اضافه شد ✓',
    chat_import_none: 'پیام تازه‌ای برای ادغام وجود نداشت',
    chat_import_preview: 'فایل شامل {n} پیام ({from} تا {to}) است که {m} تای آن تازه است. وارد کردن ادغام می‌شود و چیزی حذف نمی‌کند — ادامه می‌دهید؟',
    chat_import_same_device: '(پشتیبان همین دستگاه)',
    chat_import_newer: 'این پشتیبان از نسخهٔ جدیدتری از برنامه است',
...
I18N.register('en', {
    ...
    chat_merged: 'Merged {n} new messages ✓',
    chat_import_none: 'No new messages to merge',
    chat_import_preview: 'File has {n} messages ({from} → {to}); {m} are new. Importing merges and never deletes — add the new ones?',
    chat_import_same_device: '(same-device backup)',
    chat_import_newer: 'This backup is from a newer version of the app',
...
I18N.register('ar', {
    ...
    chat_merged: 'تم دمج {n} رسالة جديدة ✓',
    chat_import_none: 'لا توجد رسائل جديدة للدمج',
    chat_import_preview: 'يحتوي الملف على {n} رسالة ({from} ← {to})؛ {m} منها جديدة. الاستيراد يدمج ولا يحذف شيئاً — هل نضيف الجديدة؟',
    chat_import_same_device: '(نسخة احتياطية من نفس الجهاز)',
    chat_import_newer: 'هذه النسخة الاحتياطية من إصدار أحدث من التطبيق',
